...
- [x] **§77 walkthrough**: a developer who has never seen the repo opens it and
  ...
  - `docs/WALKTHROUGH.md`, generated by `scripts/render-walkthrough.py`:
      **eighteen stops** in the order somebody meeting the platform should walk
      them, each with the address, the persona to sign in as, what to do there
      and — the part a feature list cannot carry — **what to notice**, which is
      usually a decision rather than a widget. `docs/features.md` already says
      what exists and where, in the specification's numbering, which is
      nobody's reading order
  - Verified rather than trusted, because a tour that sends a reader to a page
      that does not exist is worse than no tour: the script checks every route
      against the router and every one of §77's eighteen capabilities against
      the stops (a stop claiming a capability §77 does not name fails too), and
      it runs in `make lint`
  - `frontend/e2e/walkthrough.spec.ts` **walks all eighteen in a browser as the
      persona each stop names**, asserting the page's own content rather than
      the shell — and that no stop shows a refusal. The stops are declared in
      the spec with what proves each one, and asserted to be exactly the stops
      the document lists, so the document and the test cannot drift
  - **The second half of §77 is the last stop and its own test.** "A working
      example of each capability" is satisfiable by eighteen disconnected
      demos, which is what this is not: the spec walks one thread with the
      mouse — a ticket, the account that raised it, that account's name through
      the one search box, and out into whichever dataset mentions it — because
      "reads as one application" is a claim about the links between the pages
      rather than about any of them
  - Two small things it found: the global search page's total only exists once
      there is a query (so the stop's address carries the term, which is also
